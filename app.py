...
DEFAULT_INSTRUCTIONS = (
    "Jawablah setiap soal. Soal disusun HOTS, bisa berupa pilihan ganda atau mencocokkan, mengurutkan sesuai mata pelajaran, jenjang "
...
def build_prompt(subject: str, level: str, aspiration: str, materi: str, n: int = 5) -> str:
    return f"""
Anda adalah generator bank soal untuk kuis pembelajaran adaptif.
Buat {n} soal yang menuntut HOTS (analyze/evaluate/create). Semua soal harus relevan dengan mata pelajaran "{subject}" dengan materi {materi}, jenjang "{level}", terhubung dengan cita-cita siswa: "{aspiration}".
Jenis soal hanya ada 3:
1. multiple_choice (pilihan ganda, 4 opsi A-D) jumlah 3 soal.
2. matching (mencocokkan, ada daftar kiri dan kanan yang harus dipasangkan) jumlah 1 soal.
3. ordering (mengurutkan item dalam urutan benar) jumlah 1 soal
...
- Jika ordering: gunakan field "items" berupa array string yang harus diurutkan, dan "answer" berupa array indeks urutan benar. Contoh: "items": ["Langkah 1", "Langkah 2", "Langkah 3"],"answer": [2,0,1]   // artinya urutan benar adalah item ke-2, lalu ke-0, lalu ke-1.
...
def generate_questions(api_key: str, subject: str, level: str, aspiration: str, materi: str, n: int=5,):
    prompt = build_prompt(subject, level, aspiration, materi, n)
    ...
    if start:
        if not api_key or not aspiration.strip():
            ...
            with st.spinner("Menghasilkan soal dari AI…"):
                qs = generate_questions(api_key, subject, level, aspiration, materi)
...
if not st.session_state.questions:
    ...
    if not st.session_state.submitted:
        ...
        if q["type"] == "multiple_choice":
            ...
            items = q.get("items", [])
            st.write("Urutkan item berikut sesuai jawaban yang benar:")
            selected = st.multiselect("Pilih urutan:", items, key=f"order_{idx}")
            # Simpan jawaban sebagai urutan indeks
            st.session_state.answers[idx] = [items.index(s) for s in selected] if selected else []
        ...
        for i,(q,ans,is_ok) in enumerate(zip(st.session_state.questions, st.session_state.answers, correct_flags)):
            ...
            if q["type"] == "multiple_choice":
                ...
                items = q.get("items", [])
                correct = [items[i] for i in q.get("answer", [])]
                user = [items[i] for i in (ans or [])]
                st.write("Urutan benar:")
                st.write(" → ".join(correct))
