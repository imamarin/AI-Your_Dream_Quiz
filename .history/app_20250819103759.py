...
Semua soal harus relevan dengan mata pelajaran "{subject}", jenjang "{level}", dan terhubung dengan cita-cita siswa: "{aspiration}". jika soal bertipe gambar cari dulu gambarnya baru deskripsikan soalnya
...
- Untuk soal bertipe "image", sertakan field "image_url" yang MERUJUK ke gambar publik. Pastikan URL langsung ke file gambar (jpg/png/webp) jika memungkinkan.
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("Kunci API Google Generative Language")
...
def cek_tipe_gambar(image_data):
    if isinstance(image_data, str):
        if image_data.startswith("http"):
            return "url"
        elif image_data.startswith("data:image"):
            return "base64"
        elif image_data.endswith((".png", ".jpg", ".jpeg", ".gif", ".bmp")):
            return "path lokal"
        else:
            return "string lain (mungkin teks)"
    elif isinstance(image_data, bytes):
        return "bytes (mungkin hasil decode base64)"
    else:
        return f"tipe lain: {type(image_data)}"
    ...
    if not st.session_state.submitted:
        ...
        if q.get("type", "text").lower() == "image" and q.get("image_url"):
            try:
                ...
                st.write("Jenis data gambar:",q["image_url"])
