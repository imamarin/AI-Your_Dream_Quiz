...
def build_prompt(subject: str, level: str, aspiration: str, n: int = 10) -> str:
    """
...
Buat {n} soal pilihan ganda (4 opsi: A, B, C, D) yang menuntut HOTS (analyze/evaluate/create) dan/atau menggunakan gambar.
Semua soal harus relevan dengan mata pelajaran "{subject}", jenjang "{level}", dan terhubung dengan cita-cita siswa: "{aspiration}".

Persyaratan:
- Minimal 4 dari {n} soal bertipe "image" (menggunakan gambar kontekstual). Jika sulit, boleh gunakan ilustrasi/infografik yang tersedia online.
...
def generate_questions(api_key: str, subject: str, level: str, aspiration: str, n: int = 10) -> List[Dict[str, Any]]:
        ...
        if not api_key:
            ...
            try:
                with st.spinner("Menghasilkan soal dari AI…"):
                    qs = generate_questions(api_key, subject, level, aspiration, n=10)
...
if not st.session_state.questions:
    ...
    if not st.session_state.submitted:
        ...
        if q.get("type", "text").lower() == "image" and q.get("image_url"):
            try:
                st.image(q["image_url"], caption="Perhatikan gambar berikut", use_column_width=True)
            except Exception:
        ...
        for i, (q, ans, is_ok) in enumerate(zip(st.session_state.questions, st.session_state.answers, correct_flags)):
            ...
            if q.get("type","text").lower() == "image" and q.get("image_url"):
                try:
                    st.image(q["image_url"], caption="Gambar pada soal", use_column_width=True)
                except Exception:
                    pass
